...
export default function UnderstandingPage() {
  ...
  const runUnderstanding = useCallback(async () => {
    ...
    try {
      ...
      if (aiResult) {
        ...
        setErrorMessage('理解结果为空，请重试。');
      }
    } catch {
      setErrorMessage('理解失败，请检查模型配置后重试。');
    } finally {
      setIsLoading(false);
    }
  }, [project]);
  ...
  return (
    <main className="flow-page">
      ...
      <section className="panel flow-section">
        <h3>关键信息</h3>
        <p>以下内容会优先进入结构组织与版式生成。</p>
        <ul>
          {understanding.keyPoints.map((item) => (
            <li key={item}>{item}</li>
          ))}
        </ul>
      </section>
      <section className="panel flow-section">
        <h3>风险与缺失</h3>
        <p>需要你确认或补充的点会在这里展示。</p>
        <ul>
          {understanding.openQuestions.length > 0 ? understanding.openQuestions.map((item) => (
            <li key={item}>{item}</li>
          )) : <li>暂未发现明显缺失，可继续下一步。</li>}
        </ul>
      </section>
      <section className="panel flow-section">
        <h3>系统建议</h3>
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <button type="button" onClick={() => void runUnderstanding()} disabled={isLoading}>
          {isLoading ? '理解中...' : '重新理解'}
        </button>
        <button
          onClick={() => {
